    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            if not os.path.exists('extended_user_features_5000.csv'):
                self.stdout.write(self.style.ERROR("❌ Features CSV file not found."))
                return
            ...
            X = df[['logins', 'fails', 'msgs', 'uploads', 'rate_limits']]  # Correct columns
            ...
            # Save alerts and print each suspicious user
            if suspicious_users:
                with open('alerts.log', 'w') as alert_file:
                    for user_info in suspicious_users:
                        ...
                        # 🔥 Print to console
                        self.stdout.write(self.style.WARNING(alert_text.strip()))
